...
import { FaFacebookF, FaYoutube, FaInstagram, FaEnvelope, FaPhoneAlt } from "react-icons/fa";
...
const locationSlug = (label: string) => {
  const city = label.toLowerCase().match(/(lahore|karachi|islamabad|dubai)$/)?.[1];
...
const menus = {
  what: ["WEDDING PHOTOGRAPHY", "FEMALE PHOTOGRAPHER", "PRE-WEDDING SHOOTS", "CORPORATE PHOTOGRAPHY", "BRIDAL SHOOT", "COUPLE SHOOT", "OUTDOOR SHOOT"],
  about: ["PACKAGES", "STORIES"],
  location: ["WEDDING PHOTOGRAPHER IN LAHORE", "WEDDING PHOTOGRAPHER IN KARACHI", "WEDDING PHOTOGRAPHY IN ISLAMABAD", "DUBAI"],
  gallery: ["CREATIVE", "LIGHT AND BRIGHT", "DARK AND DRAMATIC", "DESTINATION WEDDING", "FILM MAKING"],
};
...
export default function Home() {
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      gsap.fromTo(".hero-gallery-btn",
        { x: -80, opacity: 0 },
        { x: 0, opacity: 1, duration: 0.9, ease: "power3.out", delay: 0.9 }
      );
      ...
      gsap.utils.toArray<HTMLElement>(".gallery-masonry-item, .service-card, .styles article, .faq-item, .footer-col").forEach((item) => {
        gsap.from(item, {
          y: 35,
          opacity: 0,
          duration: 0.7,
      ...
      gsap.utils.toArray<HTMLElement>(".gallery-masonry-item, .service-img-wrapper, .styles article, .story img").forEach((item) => {
        const image = item.matches("img") ? item : item.querySelector("img");
        if (!image) return;

        const enter = () => {
          gsap.to(item, { y: -5, duration: 0.35, ease: "power2.out" });
          gsap.to(image, { scale: 1.06, filter: "brightness(1.08)", duration: 0.55, ease: "power3.out" });
        };
        const leave = () => {
          gsap.to(item, { y: 0, duration: 0.4, ease: "power2.out" });
          gsap.to(image, { scale: 1, filter: "brightness(1)", duration: 0.55, ease: "power3.out" });
        };
  ...
  const zoomImage = (event: MouseEvent<HTMLElement>, scale: number) => {
    const target = event.currentTarget.querySelector("img");
    if (target) gsap.to(target, { scale, duration: 0.55, ease: "power2.out" });
  };
  ...
  return (
    <main className="relative min-h-screen bg-black text-white selection:bg-white selection:text-black font-sans">
      {/* Landing Section with Background Video */}
      <section
        ...
        style={{
          backgroundImage: `url(${image("compressed_IMG_1487-F-1.jpg")})`,
        }}
      >
        <video
          ...
          aria-hidden="true"
        >
          <source src="/backgorund_video.mp4" type="video/mp4" />
        </video>
          ...
          <div className="topbar hidden lg:flex">
            ...
            <div className="topbar-contact">
              <a href="mailto:waseemsaleemphotography@gmail.com">
                ...
                <span>WASEEMSALEEMPHOTOGRAPHY@GMAIL.COM</span>
              </a>
        ...
        {/* Hero Copy (Matched Exactly to Image Position & Layout) */}
        <div className="hero-content-wrapper z-10">
          <div className="hero-copy">
            ...
            <p className="hero-subtitle">
              Waseem Saleem Photography offers cinematic &amp; timeless wedding photography, capturing love, joy, and memories with creative elegance and storytelling.
            </p>
            <a className="hero-gallery-btn" href="#gallery">
      ...
      <section className="gallery-intro reveal-section py-20 px-6 max-w-[1400px] mx-auto text-center" id="gallery">
        <h2 className="text-3xl font-serif tracking-widest mb-4 uppercase">PHOTO GALLERY</h2>
        <p className="text-sm text-gray-400 max-w-3xl mx-auto mb-2 leading-relaxed">
          Your wedding day will always be one that you remember with immense fondness, and the best wedding photography in Lahore is offered by us to make sure you immortalize that special day in the best way possible. As some of the best wedding photographers in Lahore, we take care to make sure all your smiles, gazes, and emotions are captured through cinematography.
        </p>
        ...
        <div className="gallery-masonry">
          {gallery.map(([src, alt]) => (
            <div
              ...
              onMouseEnter={(event) => zoomImage(event, 1.07)}
              onMouseLeave={(event) => zoomImage(event, 1)}
            >
              <img src={image(src)} alt={alt} loading="eager" />
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Services Section */}
      <section className="services-home reveal-section bg-white text-black py-20 px-6" id="services">
        <div className="services-container max-w-[1300px] mx-auto text-center">
          <h2 className="text-3xl font-serif tracking-widest mb-14 text-black uppercase">OUR SERVICES</h2>
          <div className="service-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {services.map((item) => (
              <article key={item.title} className="service-card flex flex-col items-center text-center bg-white">
                <div className="service-img-wrapper w-full h-[280px] overflow-hidden mb-6">
                  <img src={image(item.image)} alt={item.title} className="w-full h-full object-cover" />
                </div>
                <div className="service-content flex flex-col items-center flex-grow w-full px-2">
                  <h3 className="font-serif text-2xl tracking-wide text-black mb-4 uppercase max-w-[260px] leading-snug">{item.title}</h3>
                  <p className="text-xs md:text-sm text-gray-600 leading-relaxed mb-8 max-w-[360px]">{item.text}</p>
                  <a href="#contact" className="service-btn mt-auto bg-black text-white text-[10px] font-bold tracking-widest px-6 py-3.5 uppercase hover:bg-neutral-800 transition-colors inline-block">
                    {item.linkText}
                  </a>
                </div>
              </article>
      ...
      {/* Story Section */}
      <section className="story reveal-section py-20 px-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center" id="about">
        <div>
          <h2 className="text-2xl md:text-4xl font-light tracking-widest leading-snug mb-6">
            EVERY MOMENT DESERVES<br />TO BE REMEMBERED FOREVER
          </h2>
          <p className="text-sm text-gray-400 leading-relaxed mb-8">
            Your love story is made of the little glances, loud laughter, and beautiful chaos you never want to forget. We turn those honest moments into timeless photographs you can relive for generations.
          </p>
          <a className="outline-button border border-white px-6 py-3 text-xs tracking-widest hover:bg-white hover:text-black transition-all inline-block" href="#contact">
            LET YOUR MOMENT BLOOM FOREVER
          </a>
        </div>
        <div>
          <img src={image("Waseem-Saleem-home-story-768x997.webp")} alt="Wedding photography story" className="w-full h-auto object-cover" />
        </div>
      </section>
      ...
      <section className="styles reveal-section py-20 px-6 bg-zinc-950">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-2xl md:text-3xl font-light tracking-widest mb-12">WEDDING PHOTOGRAPHY STYLE</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <article>
              <img src={image("MGL9816-2-web-768x512.webp")} alt="Cinematic" className="w-full h-72 object-cover mb-4" />
              <h3 className="text-sm tracking-widest font-medium">CINEMATIC</h3>
            </article>
            <article>
              <img src={image("MGL9434-683x1024.jpg")} alt="Light and bright" className="w-full h-72 object-cover mb-4" />
              <h3 className="text-sm tracking-widest font-medium">LIGHT &amp; BRIGHT</h3>
            </article>
            <article>
              <img src={image("MGL0576-F-F-min-1536x1024-2-768x512.webp")} alt="Dark and dramatic" className="w-full h-72 object-cover mb-4" />
              <h3 className="text-sm tracking-widest font-medium">DARK &amp; DRAMATIC</h3>
            </article>
          </div>
        </div>
      </section>
      ...
      {/* FAQ Section */}
<section className="faq-section reveal-section bg-[#f8f8f8] text-black py-28 px-8 md:px-16" id="faq">
  <div className="faq-container max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_1.8fr] gap-16 lg:gap-24 items-start">
          ...
          >
            ...
            >
              ...
              <span className="faq-icon-circle w-9 h-9 rounded-full border border-black flex items-center justify-center flex-shrink-0 transition-transform duration-200">
                {isOpen ? <FiMinus className="text-base" /> : <FiPlus className="text-base" />}
              </span>
            </button>
            {isOpen && (
    ...
    {/* Footer Section */}
<footer className="site-footer reveal-section bg-black text-white pt-24 pb-12 px-8 border-t border-white/10" id="contact">
  <div className="footer-container max-w-[1350px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 text-center mb-20 items-start">
    ...
    <div className="footer-col flex flex-col items-center">
      ...
      <p className="text-base text-gray-200 mb-4 tracking-wide font-light">
        <a href="mailto:waseemsaleemphotography@gmail.com" className="hover:underline">
          waseemsaleemphotography@gmail.com
        </a>
      </p>
      <p className="text-base text-gray-200 mb-2 tracking-wide font-light">+92 309-925 2015</p>
      <p className="text-base text-gray-200 tracking-wide font-light">+92 304-805 5553</p>
    </div>
    ...
    <div className="footer-col flex flex-col items-center">
      ...
      <p className="text-base text-gray-200 max-w-[320px] leading-relaxed mb-6 font-light">
        We providing the best photography services in Major Areas.
      </p>
      <p className="text-base text-gray-200 mb-2 tracking-wide font-light">Lahore – Islamabad</p>
      <p className="text-base text-gray-200 tracking-wide font-light">Karachi</p>
    </div>
  ...
  <div className="footer-bottom max-w-[1350px] mx-auto text-center flex flex-col items-center gap-3 pt-10 border-t border-white/10 text-sm md:text-base text-gray-300 tracking-wide font-light">
    <p>Copyright © 2026 – Waseem Saleem Photography | Terms &amp; Conditions | Privacy Policy</p>
    <p className="text-gray-300">
      Designed By <a href="#" className="text-white underline hover:text-gray-200 font-normal">Sufyan Malik</a>
    </p>
  </div>
</footer>
    </main>
